...
##### contracts
Содержит интерфейсы
- UserService - сервис пользователей, позволяет зарегистировать нового пользователя, либо авторизоваться как существующий пользователь 
- WalletService - сервис кошельков, позволяет сгенерировать новый кошелёк, сохранить его и получить существующий кошелёк по его идентификатору
...
- Все входящие параметры кроме пароля пользователя валидируются
- Пользователю разрешено создавать транзакции с нулевой суммой
- Пользователю разрешено устанавливать бюджеты с нулевой суммой
---
...
- exit - закрывает приложение, сохраняет статус активного кошелька
- add_expense - запускает процесс добавления расходной транзакции. Принимает на вход категорию и сумму расхода. В случае, если для данной категории установлени бюджет, будет осуществлена проверка на его превышение. Если имеет место превышение бюджета, пользователю будет выведено уведомление. Если бюджет не превышен, но сумма расходов приближается к лимиту, пользователю будет выведено другое уведомление 
- add_income - запускает процесс добавления доходной транзации. Принимает на вход категорию и сумму 
...
- set_budget - запускает процесс установки бюджета для определённой категории. Принимает на вход категорию и сумму. Если установлена нулевая сумма, пользователю будет выведено предупреждение, но это не будет считаться ошибкой
...
- export_wallet - экспортирует данные текущего активного кошелька в формате JSON
