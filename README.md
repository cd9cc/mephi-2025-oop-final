#### Сборка
В качестве системы автоматизации сборки используется Gradle. В проекте реализована автоматическое тестирование и сборка с помощью GitHub Actions. 
Настройки CI находятся в файле .github/workflows/gradle.yml. Пайплайн срабатывает при принятии pull request в ветку master.
...
- CliContoller - контроллер для работы с вводом/выводом через командную строку. Является точкой входа в приложение для конечного пользователя
##### exceptions
...
- NegativeMoneyException - попытка создать объект Money с отрицательной денежной суммой
...
- Money - всегда положительная сумма в копейках 
...
- Wallet - кошелёк пользователя, содержит данные о доходах, расходах и установленных бюджетах для категорий, а также методы для работы с ними
...
```
└───sessionapp
        BudgetSpendingResultTest.java
        CryptoUtilsTest.java
        EncryptedFileWalletServiceTest.java
        InMemoryUserServiceTest.java
        JSONWalletExporterTest.java
        MoneyTest.java
        TestUtils.java
        UserTest.java
        WalletTest.java
...
8. Файлы пользовательских кошельков хранятся на диске в зашифрованном виде и расшифровываются в момент аутентификации пользователя. Все изменения сохраняются в реальном времени, а также при выходе из приложения
9. Валидация ввода пользователя для каждой операции
